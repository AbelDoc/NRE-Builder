    ...
    namespace NREB {
        namespace IO {
            ...
            void Folder::createArchiverScript(FileList& files) const {
                std::ofstream archiver(ARCHIVER);
                archiver << "#!/bin/bash\n";

                std::string command(Config::Config::getConfig().getArchiver() + " cr ");

                std::string out(Config::Config::getConfig().getOut());
                std::string path = "";
                std::size_t pos = out.find("/");
                while (pos != std::string::npos) {
                    path += out.substr(0, pos + 1);
                    out.erase(0, pos + 1);
                    pos = out.find("/");
                }
                out = "lib/" + path + "lib" + out + ".a ";
                command += out;

                for (File* f : files) {
                    command += "obj/" + f->getObjectPath() + " ";
                }
                archiver << command;
            }
            ...
            bool Folder::isExcluded(std::string const& path) {
                std::string parsedPath = path;
                std::size_t pos = parsedPath.find("\\");
                std::string replace("/");
                while (pos != std::string::npos) {
                    parsedPath.replace(pos, replace.length(), replace);
                    pos = parsedPath.find("\\");
                }
                for (std::string const& ex : Config::Config::getConfig().getExcluded()) {
                    if (parsedPath.find(ex) != std::string::npos) {
                        return true;
                    }
                }
                return false;
